...
function buildDailyPrompt(p: Record<string, string>, t: Record<string, string>, recentThemes: string[] = [], sources: Array<{name: string, url?: string, desc?: string}> = []): string {
  ...
  const sourcesSection = sources.length
    ? `\n【ユーザーが日常的に参照している情報ソース】\nこれらのソースに関連したテーマ・具体的なコンテンツを今日の提案に反映してください:\n${sources.map(s => `・${s.name}${s.url ? ` (${s.url})` : ''}${s.desc ? ` ※${s.desc}` : ''}`).join('\n')}\n`
    : ''
...
serve(async (req) => {
  ...
  try {
    ...
    // リクエスト内容を取得
    const { type, profile, target, recentThemes, sources } = await req.json()

    let prompt = ''
    if      (type === 'statement') prompt = buildStatementPrompt(profile, target)
    else if (type === 'sns')       prompt = buildSnsPrompt(profile, target)
    else if (type === 'daily')     prompt = buildDailyPrompt(profile, target, recentThemes ?? [], sources ?? [])
